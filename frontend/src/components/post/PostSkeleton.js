export default function PostSkeleton() {
  return (
    <div className="flex flex-col items-center justify-center w-full overflow-hidden h-full px-0 sm:px-1 lg:px-4 gap-4">
      <div className="bg-white rounded-lg shadow-lg w-full px-2 sm:px-4 py-2">
        <div
          role="status"
          class="space-y-8 animate-pulse md:space-y-0 md:space-x-8 flex flex-col  items-center justify-center"
        >
          <div className=" pb-2 pt-1 w-full flex flex-col justify-between gap-2">
            <div className="w-full flex flex-row gap-2">
              <div className="flex flex-row items-center gap-2">
                <svg
                  class="w-10 h-10 rounded-full text-gray-200 dark:text-gray-600"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 20 18"
                >
                  <path d="M18 0H2a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2Zm-5.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Zm4.376 10.481A1 1 0 0 1 16 15H4a1 1 0 0 1-.895-1.447l3.5-7A1 1 0 0 1 7.468 6a.965.965 0 0 1 .9.5l2.775 4.757 1.546-1.887a1 1 0 0 1 1.618.1l2.541 4a1 1 0 0 1 .028 1.011Z" />
                </svg>
              </div>
              <div className="gap-1 w-full flex flex-col justify-center">
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700 w-48"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700"></div>
              </div>
            </div>
            <div className="w-full flex flex-col gap-2 mt-2">
              <div className="gap-1 w-full flex flex-col justify-center">
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700 w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700 w-48"></div>
              </div>
            </div>
          </div>

          <span class="sr-only">Loading...</span>
        </div>
      </div>
      <div className="bg-white rounded-lg shadow-lg w-full px-2 sm:px-4 py-2">
        <div
          role="status"
          class="space-y-8 animate-pulse md:space-y-0 md:space-x-8 flex flex-col  items-center justify-center"
        >
          <div className=" pb-2 pt-1 w-full flex flex-col justify-between gap-2">
            <div className="w-full flex flex-row gap-2">
              <div className="flex flex-row items-center gap-2">
                <svg
                  class="w-10 h-10 rounded-full text-gray-200 dark:text-gray-600"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 20 18"
                >
                  <path d="M18 0H2a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2Zm-5.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Zm4.376 10.481A1 1 0 0 1 16 15H4a1 1 0 0 1-.895-1.447l3.5-7A1 1 0 0 1 7.468 6a.965.965 0 0 1 .9.5l2.775 4.757 1.546-1.887a1 1 0 0 1 1.618.1l2.541 4a1 1 0 0 1 .028 1.011Z" />
                </svg>
              </div>
              <div className="gap-1 w-full flex flex-col justify-center">
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700 w-48"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700"></div>
              </div>
            </div>
            <div className="w-full  flex flex-col gap-2 mt-2">
              <div className="gap-1 px-2 w-full flex flex-col justify-center">
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700 w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700 w-48"></div>
              </div>
              <div className="flex flex-row items-center gap-1">
                <svg
                  class="w-full text-gray-300 dark:text-gray-600 "
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 20 18"
                >
                  <path d="M18 0H2a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2Zm-5.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Zm4.376 10.481A1 1 0 0 1 16 15H4a1 1 0 0 1-.895-1.447l3.5-7A1 1 0 0 1 7.468 6a.965.965 0 0 1 .9.5l2.775 4.757 1.546-1.887a1 1 0 0 1 1.618.1l2.541 4a1 1 0 0 1 .028 1.011Z" />
                </svg>
                <svg
                  class="w-full text-gray-300 dark:text-gray-600 "
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 20 18"
                >
                  <path d="M18 0H2a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2Zm-5.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Zm4.376 10.481A1 1 0 0 1 16 15H4a1 1 0 0 1-.895-1.447l3.5-7A1 1 0 0 1 7.468 6a.965.965 0 0 1 .9.5l2.775 4.757 1.546-1.887a1 1 0 0 1 1.618.1l2.541 4a1 1 0 0 1 .028 1.011Z" />
                </svg>
              </div>
            </div>
          </div>

          <span class="sr-only">Loading...</span>
        </div>
      </div>
      <div className="bg-white rounded-lg shadow-lg w-full px-2 sm:px-4 py-2">
        <div
          role="status"
          class="space-y-8 animate-pulse md:space-y-0 md:space-x-8 flex flex-col  items-center justify-center"
        >
          <div className=" pb-2 pt-1 w-full flex flex-col justify-between gap-2">
            <div className="w-full flex flex-row gap-2">
              <div className="flex flex-row items-center gap-2">
                <svg
                  class="w-10 h-10 rounded-full text-gray-200 dark:text-gray-600"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 20 18"
                >
                  <path d="M18 0H2a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2Zm-5.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Zm4.376 10.481A1 1 0 0 1 16 15H4a1 1 0 0 1-.895-1.447l3.5-7A1 1 0 0 1 7.468 6a.965.965 0 0 1 .9.5l2.775 4.757 1.546-1.887a1 1 0 0 1 1.618.1l2.541 4a1 1 0 0 1 .028 1.011Z" />
                </svg>
              </div>
              <div className="gap-1 w-full flex flex-col justify-center">
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700 w-48"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700"></div>
              </div>
            </div>
            <div className="w-full flex flex-col gap-2 mt-2">
              <div className="gap-1 w-full flex flex-col justify-center">
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700 w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-96"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-full"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700  w-96"></div>
                <div class="h-2 bg-gray-300 rounded-full dark:bg-gray-700 w-48"></div>
              </div>
            </div>
          </div>

          <span class="sr-only">Loading...</span>
        </div>
      </div>
    </div>
  );
}
