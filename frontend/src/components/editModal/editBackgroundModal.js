...
export default function EditBackgroundModal(props) {
  ...
  const handleSubmitImage = async (image) => {
    toast.promise(
      updateUserData({ background: image, username: user.username }),
      {
                  ...
                  <div className="flex items-center gap-2">
                    <FontAwesomeIcon
                      ...
                      onClick={() => {
                        ...
                        setImageSource(user.background);
                        setZoom(1);
                        setRotation(0);
                      }}
                    />
